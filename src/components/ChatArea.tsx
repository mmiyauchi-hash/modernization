...
const stepHelpGuides: Record<string, { title: string; description: string; steps?: string[]; tips?: string[] }> = {
  ...
  'system-name': {
    title: '⚠️ システム名の入力【重要】',
    description: '移行するシステムの名前を入力します。この名前はリポジトリ名の一部になるため、命名規則に従って慎重に入力してください。',
    steps: [
      '① システムの正式名称を確認してください',
      '② 半角英数字とハイフン(-)のみ使用可能です',
      '③ スペースは使用できません',
      '④ 例: inventory-system, sales-portal, hr-management',
      '⑤ テキストボックスに入力して送信してください',
    ],
    tips: [
      '【システム名とは？】プロジェクトやアプリケーションを識別する名前です',
      '【入力例】inventory-system, customer-portal, expense-tracker',
      '【注意】入力した名前は後から変更が難しいため、慎重に決めてください',
      '【命名規則】会社の命名規則がある場合はそれに従ってください',
    ],
  },
...
// system-name: システム名入力 - 命名規則があり、後から変更が難しいため重要
// repository-name: リポジトリ名入力 - 社内命名規則がある場合は特に注意が必要
// ssh-key-generation: SSH鍵生成 - 技術的に複雑で、初心者がつまづきやすい
const AUTO_HELP_STEPS = ['system-name', 'repository-name', 'ssh-key-generation'];
